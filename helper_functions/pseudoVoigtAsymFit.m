function [a,b,da,db,chi2,Rwp,S,C,n] = pseudoVoigtAsymFit(x,y,w,a0,b0,mu,Linda,Lindb)
%-----------------------------------------------------------------
% PSEUDOVOIGTASYMFIT    Fit data by a sum of asymmetric 
%                       pseudo-Voigt functions and a polynomial
%                       background using a simple least squares
%                       algorithm.
%
% [a,b,chi2,Rwp,S,da,db,C,n]
%             = pseudoVoigtAsymFit(x,y,w,a0,b0,mu,Linda,Lindb)
%
% x     - x data points (a row vector)
% y     - y data points (a row vector)
% w     - data weights (a row vector)
% a0    - starting parameters of asym. pseudo-Voigt functions,
%         a (n x 5) matrix, where n is the number
%         of asym. pseudo-Voigt functions. For more information
%         see a 'pseudoVoigtAsym' function.
...
%         asym. pseudo-Voigt functions parameters
% Lindb - logical vector (same size as b0) specifing refined
%         parameters of the background polynomial
% 
% a     - matrix of refined asym. pseudo-Voigt functions parameters
% b     - vector of refined parameters of the background polynomial
% da    - matrix of esds of refined asym. pseudo-Voigt functions
%         parameters
% db    - vector of esds of refined parameters of the background
%         polynomial
% chi2  - chi2 value
...
%            % create data
%            x = [25:0.05:40];
%            a = [400 33 0.2 0.5 2.0; ...
%                 500 31 0.4 0.4 1.0];
%            b = [400 -350 200];
%            mu(1) = (x(1)+x(end))/2;
%            mu(2) = x(end)-x(1);
%            y = sum(pseudoVoigtAsym(a,x),1)+polyval(b,x,[],mu);
%            y = y+sqrt(y).*randn(1,length(y));
%            plot(x,y,'k*') 
%            % initial guess
%            a0 = [600 33.15 0.3 0.5 1.0; ...
%                  600 30.92 0.3 0.5 1.0];
%            Linda = [1 1 1 1 1;
%                     1 1 1 1 0];
%            b0 = [0 0 100];
%            Lindb = [1 1 1];
%            yc = sum(pseudoVoigtAsym(a0,x),1)+polyval(b0,x,[],mu);
%            hold on
%            plot(x,yc,'g')
%            % fitting
%            w = 1./y;
%            [a,b] = pseudoVoigtAsymFit(x,y,w,a0,b0,mu,Linda,Lindb)
%            yc = sum(pseudoVoigtAsym(a,x),1)+polyval(b,x,[],mu);
%            plot(x,yc,'b')
%              
% For more detailed information see a function 'pseudoVoigtAsym'. 
%
% X-ray diffraction: see the 'pseudoVoigtAsym' function if you
...
%
% See also pseudoVoigt, pseudoVoigtAsym, pseudoVoigtAsymDeriv 
%
...
if size(a0,2)~=5 a0 = reshape(a0,[],5); end
if ~exist('Linda','var') | isempty(Linda) Linda = ones(size(a0)); end
if size(Linda,2)~=5 Linda = reshape(Linda,[],5); end    
...
for n=1:100
    ...
    % calc new value chi2
    yc = s*( sum(pseudoVoigtAsym(a,x),1) + polyval(b,x,[],mu) );
    chi20 = chi2;
    chi2 = (y-yc).^2*w';
